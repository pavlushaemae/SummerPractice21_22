...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val weight: TextInputLayout? = findViewById(R.id.et_weight)
        val height: TextInputLayout? = findViewById(R.id.et_height)
        val age: TextInputLayout? = findViewById(R.id.et_age)
        val name: TextInputLayout? = findViewById(R.id.et_name)
        val result: TextView? = findViewById(R.id.te_result)
        button?.setOnClickListener {
            try {
                val weightNumber = weight?.editText.toString().toFloat()
                val heightNumber = height?.editText.toString().toInt()
                val ageNumber = age?.editText.toString().toInt()
                val lengthOfName = name?.editText.toString().count()
